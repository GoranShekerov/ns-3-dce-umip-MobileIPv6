...
void MainWindow::on_run_pb_clicked()
{
    ...
    else
    {
        ...
        while ( dep <= dep_last)
        {
            ...
            if ( ui->dependency_combo->currentIndex() == 2) {ui->V_value->setText(QString::number(dep));}
...
// Packet loss [%]
    ...
    // is confidence level Ok
    bool pl_95 = (pl_error <= pl_confidence_interval) ? true : false;
    if (pl_95)
    {
...
// Jitter [ms]
    ...
    // is confidence level Ok
    bool j_95 = (j_error <= j_confidence_interval) ? true : false;
    if (j_95)
    {
